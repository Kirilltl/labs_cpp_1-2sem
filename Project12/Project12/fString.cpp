...
#include <iostream>
using namespace std;
unsigned int length_word(const char* str) // dlina stroki
{
	int res = 0;
...
void MyString::del_substr(unsigned int start, unsigned int end)
{
	...
	if (!tmp)
		exit(1);
	int j = 0;
	for (unsigned int i = 0; i < length_word(m_str) + 1; i++) // и был интовым и было предупреждение несовместимости поэтому и теперь unsigned
	{
		if (i >= start && i <= end)
			continue;
		tmp[j] = m_str[i];
...
MyString& MyString::operator+(MyString& s)//конкатенация
{
